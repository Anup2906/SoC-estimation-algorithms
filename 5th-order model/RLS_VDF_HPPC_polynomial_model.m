...
percentage_error = 100 * (tracking_error(valid_idx) ./ Vt(valid_idx));
figure;
...
I = smooth(I, 5);  % Simple 5-point smoothing
Vt = smooth(Vt, 5);  % Or use a filter like filtfilt with Butterworth
...
for k = 2:length(time)
    ...
    dVoc_dSoC = max(min(polyval(polyder(p), x_pred(1)), 10), 0);  % Clamp slope
    H = [dVoc_dSoC, -1];
