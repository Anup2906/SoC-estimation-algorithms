...
soc_true = (meas.Ah - meas.Ah(end)) / nominal_capacity;
dt = mean(diff(time));
N = length(time);
...
ocv_fit = polyval(p, soc_all);  % Fitted OCV values

% 2. Compute residuals
ocv_fit_error = ocv_all - ocv_fit;

% 3. Error Metrics
RMSE_ocv = sqrt(mean(ocv_fit_error.^2));
MAE_ocv = mean(abs(ocv_fit_error));
MAPE_ocv = mean(abs(ocv_fit_error ./ ocv_all)) * 100;
...
figure;
plot(soc_all, ocv_all, 'ko', 'DisplayName', 'Actual OCV (rest data)');
hold on;
plot(soc_all, ocv_fit, 'r-', 'LineWidth', 1.5, 'DisplayName', 'Polynomial Fit');
