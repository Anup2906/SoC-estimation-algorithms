...
% Point-wise percentage voltage error
percentage_voltage_err = abs(voltage_err ./ voltage(2:end)) * 100;

% Error metrics
RMSE_V = sqrt(mean(voltage_err.^2));
MAE_V = mean(abs(voltage_err));
MAPE_V = mean(percentage_voltage_err);  % Correct MAPE calculation

% Display results
fprintf('\n--- Voltage Tracking Error Metrics ---\n');
fprintf('RMSE Voltage = %.6f V\n', RMSE_V);
fprintf('MAE Voltage  = %.6f V\n', MAE_V);
fprintf('MAPE Voltage = %.6f %%\n', MAPE_V);
